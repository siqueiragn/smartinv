...
foreach($consultaPGSQL as $line) {echo "<option value='".$line['table_name']."'>".$line['table_name']."</option>";	}
...
foreach($consultaMYSQL as $line){	echo "<option value='".$line['COLUMN_NAME']."'>".$line['COLUMN_NAME']."</option>";	}
echo "</select>";
...
} else {	$_SESSION['col_mysql'] = '';}
echo "<br>";
if(isset($_REQUEST['columnpgsql'])){$_SESSION['col_pgsql'] .= $_REQUEST['columnpgsql'] . ', ';} else {$_SESSION['col_pgsql'] = '';}
echo "<br>";
...
for ($i=0;$i<sizeof($line)/2;$i++){	$query .= $line[$i] . ", ";}

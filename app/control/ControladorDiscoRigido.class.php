...
/**
 * Classe controladora referente ao objeto Disco_rigido para 
...
class ControladorDiscoRigido extends ControladorGeral
 {
    ...
    protected $model;
     ...
     /**
      * Construtor da classe Disco_rigido, esse método inicializa o  
      * modelo de dados 
      *
      */
    public function __construct() {
        parent::__construct();
        $this->model = new DiscoRigidoDAO();
    }
    ...
    {
        $this->view->setTitle('Disco rigido');

        Componente::carregaComponente('TabelaManterDados'); 
        $tabela = new TabelaManterDados();
        $tabela->setDados(BASE_URL . '/discoRigido/tabela');
        $tabela->setTitulo('Disco rigido');
        $tabela->addAcaoAdicionar(BASE_URL . 
        '/discoRigido/criarNovo');
        $tabela->addAcaoEditar(BASE_URL . 
        '/discoRigido/editar');
        $tabela->addAcaoDeletar(BASE_URL . 
        '/discoRigido/deletarFim');

         //Colunas da tabela
        $tabelaColuna = new TabelaColuna('Id disco rigido', 'id_disco_rigido');
        ...
        $tabelaColuna = new TabelaColuna('V cache', 'v_cache');
        $tabelaColuna->setLargura(40);
        $tabelaColuna->setBuscaTipo('integer');
        $tabela->addColuna($tabelaColuna);

        $tabelaColuna = new TabelaColuna('Rpm', 'rpm');
        $tabelaColuna->setLargura(40);
        $tabelaColuna->setBuscaTipo('integer');
        $tabela->addColuna($tabelaColuna);
    ...
    public function tabela()
      ...
      *
      * @param DiscoRigido $obj - Objeto DataTransfer com os dados da classe
      */
    public function criarNovo(DiscoRigido $obj = null)
     {
        $discoRigido = $obj == null ? new DiscoRigido() : $obj;

        $this->view->setTitle('Novo Disco rigido');

        $this->view->attValue('discoRigido', $discoRigido);

        //Carrega os campos de seleção;
        $this->getSelects();
        $this->view->startForm(BASE_URL  . '/discoRigido/criarNovoFim');
        $this->view->addTemplate('forms/disco_rigido');
        $this->view->endForm();
    }
     ...
     *
     * @param DiscoRigido $obj - Objeto para carregar os formulários 
     */
    public function editar(DiscoRigido $obj = null) 
    {
        if($obj == null){
            $id = ValidatorUtil::variavelInt($GLOBALS['ARGS'][0]);
            $discoRigido = $this->model->getById($id);
        }else{
            $discoRigido = $obj;
        }

        $this->view->setTitle('Editar Disco rigido');
        ...
        $this->view->startForm(BASE_URL . '/discoRigido/editarFim');
        $this->view->addTemplate('forms/disco_rigido');
        $this->view->endForm();
    }
    ...
    public function criarNovoFim()
     {
        ...
        try {
            unset($_POST['idDiscoRigido']);
            if($discoRigido->setArrayDados($_POST) > 0){ 
                $this->view->addErros($GLOBALS['ERROS']);
            }else{
                if($this->model->create($discoRigido)){
    ...
    public function editarFim()
     {
        $discoRigido = new DiscoRigido();
        $id = ValidatorUtil::variavelInt($_POST['idDiscoRigido']);
        $discoRigido->setIdDiscoRigido($id);
        try {
             if($discoRigido->setArrayDados($_POST) > 0){ 
                 $this->view->addErros($GLOBALS['ERROS']);
             }else{
                 if($this->model->update($discoRigido)){
    ...
    public function deletarFim()
    {
        ...
        $discoRigido->setIdDiscoRigido($id);
        try {
             if($this->model->delete($discoRigido) !== false){
